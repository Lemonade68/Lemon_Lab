...
24.dielectric实现中，注意宏观法线和wh的方向时刻与wo在同一侧；注意fr是与wo相关（摄像机发出的光线），注意与材料进行区分（wi,wo的定义，以及wi,wo的方向）

问题：为什么eta记录的很小时conductor的反射才正常？？（.15样子才正常），dielectric中也一样，反射部分只有eta很小时才正常，不然就基本是全黑
