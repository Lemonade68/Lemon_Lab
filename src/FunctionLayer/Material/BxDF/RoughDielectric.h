...
public:
    ...
    virtual Spectrum f(const Vector3f &wo, const Vector3f &wi) const override {
        ...
        Vector3f n = -normal;           //n表示当次反射/折射对应的宏观法线，初始让n指向物体内部
        //如果wi在物体外部，应该颠倒eta，让wi侧/wo侧等于外界折射率/物体折射率，并让n指向物体外部
        if(dot(wi_local, normal) > .0f){
            eta_inv = eta_;
            eta = eta_inv_;
            n = normal;
        }
        ...
        //计算wh，并使wh与入射光线wi同侧，此时n也与入射光线wi同侧
        ...
        if(isReflect){
            ...
            return albedo * (1.f - Fr) * G * D
                   * (std::abs(dot(wh_local, wo_local)) * std::abs(dot(wh_local, wi_local)))
                   / (std::abs(dot(wo_local, n)) * std::abs(dot(wi_local,n))) 
